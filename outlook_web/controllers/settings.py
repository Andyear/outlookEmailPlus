...
def api_update_settings() -> Any:
    ...
    current_webhook_notification_enabled = (
        settings_repo.get_webhook_notification_enabled()
    )
    ...
    if "webhook_notification_url" in data:
        target_webhook_notification_url = str(
            data.get("webhook_notification_url") or ""
        ).strip()

    if (
        "webhook_notification_enabled" in data
        or "webhook_notification_url" in data
        or "webhook_notification_token" in data
    ):
        ...
        if "webhook_notification_url" in data:
            queue_setting_update(
                "webhook_notification_url", target_webhook_notification_url
            )
            updated.append("Webhook URL")

        if "webhook_notification_token" in data:
            new_webhook_token = str(
                data.get("webhook_notification_token") or ""
            ).strip()
            if (
                new_webhook_token
                and current_webhook_notification_token
                and new_webhook_token
                == _mask_secret_value(current_webhook_notification_token)
            ):
                updated.append("Webhook Token（未变更）")
            elif new_webhook_token:
                queue_setting_update(
                    "webhook_notification_token", encrypt_data(new_webhook_token)
                )
    ...
    if updated:
        ...
        webhook_notification_just_enabled = (
            not current_webhook_notification_enabled
        ) and target_webhook_notification_enabled
...
def api_test_webhook() -> Any:
    ...
    log_audit(
        "webhook_notification_test", "settings", None, f"success=true url={safe_url}"
    )
    return jsonify(
        {
            "success": True,
