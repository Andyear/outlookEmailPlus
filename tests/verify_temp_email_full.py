...
def run():
    ...
    with sync_playwright() as p:
        ...
        page.locator(
            ".workspace-panel.accounts-column button:has-text('+ 创建')"
        ).click()
        try:
        ...
        all_emails_text = page.locator(
            "#tempEmailContainer .account-email"
        ).all_inner_texts()
        ...
        if cards.count() > 0:
            ...
            active_count = page.locator(
                "#tempEmailContainer .account-card.active"
            ).count()
        ...
        if curr:
            ...
            if active_card.count() > 0:
                ...
                is_cleared = (
                    "empty-state" in msg_list_html or "收件箱为空" in msg_list_html
                )
        ...
        if current_acc2 and before_del_count > 0:
            ...
            if active_card2.count() > 0:
                ...
                del_btn.click()
                page.wait_for_timeout(2500)
                after_del_count = page.locator(
                    "#tempEmailContainer .account-card"
                ).count()
