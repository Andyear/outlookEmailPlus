...
> - 2026-04-16 前端已落地新手快速指引折叠卡片（5 步）与折叠状态记忆（localStorage）
> - Scope UX 口径调整为“推荐 Graph 邮件预设”；后端环境变量层 fallback 仍保持 IMAP 兼容预设
>
...
3. 兼容导入模式下 `client_secret` 固定为空（不支持机密客户端输入）
...
| **安全优先** | PKCE 强制；兼容模式下 `client_secret` 固定为空,不参与流程 |
...
1. **配置 OAuth 参数**: 用户输入 `client_id`、`redirect_uri`、权限 scope（`tenant` 固定 `consumers`，`client_secret` 固定空）
...
| Client Secret | ❌ | 兼容模式固定为空，不支持输入 | 固定空值 |
| Tenant（租户） | ✅ | 兼容模式固定 `consumers` | 固定 `consumers` |
| Redirect URI | ✅ | 需与 Azure 注册一致 | 自动检测当前访问地址 |
| Scope（权限） | ✅ | 请求的 OAuth 权限列表（前端首次展示 Graph 预设） | 前端 Graph 预设；后端 `OAUTH_SCOPE` fallback |
| 强制 Consent | ❌ | 是否强制弹出权限确认页 | 否 |

**Tenant（租户）说明（当前实现）**:

- 当前实现固定 `tenant=consumers`，前端不暴露可变 tenant 输入。
- `common` / `organizations` / 自定义 tenant ID 属于历史设计讨论，不在当前兼容导入模式内。
...
| `AADSTS70000` | 请求被拒绝（scope 未授权或已失效） | 常见于“授权时 scope 与验证时 scope 不一致”或 Azure 应用未授予对应权限 | 提示: "请确认 Graph 必要权限已添加（offline_access / Mail.Read / User.Read），并勾选强制 Consent 后重新授权获取新 refresh_token" |
...
| `OAUTH_CLIENT_SECRET` | 环境变量 | 空 | 兼容模式下固定为空（不参与页面输入） |
| `OAUTH_REDIRECT_URI` | 环境变量 | 自动检测 | 默认的 Redirect URI |
| `OAUTH_SCOPE` | 环境变量 | `offline_access https://outlook.office.com/IMAP.AccessAsUser.All` | 后端 fallback 默认权限（前端首次展示 Graph 预设） |
| `OAUTH_TENANT` | 环境变量 | `consumers` | 兼容模式固定租户 |

**说明**:
- 兼容模式下可编辑项为 `client_id` / `redirect_uri` / `scope` / `prompt_consent`；`tenant` 固定 `consumers`，`client_secret` 固定空
...
- [ ] OAuth 参数配置表单（client_id / redirect_uri / scope，兼容模式固定 `tenant=consumers`、`client_secret` 为空）
...
- [ ] 主导航栏新增入口（可通过开关隐藏）
- [ ] 常见错误中文提示与解决建议引导
- [ ] 兼容模式约束校验（拒绝非 `consumers` tenant 与非空 `client_secret`）
...
> **v1.5 更新说明**: 当前兼容导入模式中 `client_secret` 固定为空值且不参与流程；历史机密客户端设计不再作为本期实现范围。
...
| client_secret 处理 | 兼容模式固定空值，不接收也不写入敏感密钥 |
...
- 「获取授权链接」按钮: 生成授权链接，用户可复制或在新标签页打开完成授权
...
**扩展机制**: 指引底部保留 `guide-links` 教程链接占位区，可在后续教程发布时按需追加外链。
...
- 受支持的账户类型: 选择「Accounts in any organizational directory and personal Microsoft accounts」
- ⚠️ 不要选择「仅个人 Microsoft accounts」，否则容易命中 `AADSTS9002331`
...
- 推荐直接使用系统内「Graph 邮件」预设（系统默认邮件读取主链路）

**步骤 4: 配置重定向 URI**
- 在「身份验证」页面添加「Web」平台回调地址
- 本地部署示例: `http://localhost:5001/token-tool/callback`（端口按实际部署）
- 远程/Docker 场景可使用 `http://localhost`，授权后手动复制回调 URL 粘贴回工具
...
| A-08 | 兼容模式约束 | 页面/API 均固定 `tenant=consumers` 且拒绝非空 `client_secret` |
